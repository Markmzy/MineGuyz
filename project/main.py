...
class QNetwork(nn.Module):
    ...
    def __init__(self, obs_size, action_size, hidden_size=150):

        super().__init__()
        self.net = nn.Sequential(
                                nn.Linear(np.prod(obs_size), hidden_size),
                                nn.ReLU(),
...
def init_malmo(agent_host):
    ...
    """
    my_mission = MalmoPython.MissionSpec(GetMissionXML(SIZE, OBS_SIZE, MAX_EPISODE_STEPS), True)
